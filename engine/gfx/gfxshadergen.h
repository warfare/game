...
void gfxAddVertexAttributes(gfxDocument* document, gfxShaderConfig* config)
{
	...
	if(config->light || config->textures & GFX_HEIGHT_MAP || config->textures & GFX_CUBE_MAP) gfxAddDocumentLine(document, "attribute vec3 elf_NormalAttr;");
	if(config->textures) gfxAddDocumentLine(document, "attribute vec2 elf_TexCoordAttr;");
	if((config->light && config->textures & GFX_NORMAL_MAP) || config->textures & GFX_HEIGHT_MAP) gfxAddDocumentLine(document, "attribute vec3 elf_TangentAttr;");
	if(config->vertexColor) gfxAddDocumentLine(document, "attribute vec4 elf_ColorAttr;");
}
...
void gfxAddVertexUniforms(gfxDocument* document, gfxShaderConfig* config)
{
	...
	if(config->textures & GFX_CUBE_MAP) gfxAddDocumentLine(document, "uniform vec3 elf_CameraPosition;");
}

void gfxAddVertexVaryings(gfxDocument* document, gfxShaderConfig* config)
{
	if(config->light || config->textures & GFX_HEIGHT_MAP) gfxAddDocumentLine(document, "varying vec3 elf_EyeVector;");
	if(config->light && config->textures & GFX_NORMAL_MAP) gfxAddDocumentLine(document, "varying vec3 elf_LightTSDirection;");
	if(config->light && !(config->textures & GFX_NORMAL_MAP)) gfxAddDocumentLine(document, "varying vec3 elf_Normal;");
	if(config->light) gfxAddDocumentLine(document, "varying vec3 elf_LightDirection;");
	if(config->light && config->light != GFX_SUN_LIGHT) gfxAddDocumentLine(document, "varying float elf_Distance;");
	if(config->light && config->textures & GFX_SHADOW_MAP) gfxAddDocumentLine(document, "varying vec4 elf_ShadowCoord;");
	if(config->textures && config->textures != GFX_SHADOW_MAP) gfxAddDocumentLine(document, "varying vec2 elf_TexCoord;");
	if(config->textures & GFX_CUBE_MAP) gfxAddDocumentLine(document, "varying vec3 elf_CubeMapCoord;");
	if(config->vertexColor) gfxAddDocumentLine(document, "varying vec4 elf_VertexColor;");
}
...
{
	if(config->textures && config->textures != GFX_SHADOW_MAP) gfxAddDocumentLine(document, "\telf_TexCoord = elf_TexCoordAttr;");
	if(config->light && config->textures & GFX_SHADOW_MAP) gfxAddDocumentLine(document, "\telf_ShadowCoord = elf_ShadowProjectionMatrix*vertex;");
}
...
void gfxAddVertexLightingCalcs(gfxDocument* document, gfxShaderConfig* config)
{
	...
	if(config->light)
	{
		...
		{
			if(config->light != GFX_SUN_LIGHT) gfxAddDocumentLine(document, "\ttmpvec = elf_LightPosition-vertex.xyz;");
			if(config->light == GFX_SUN_LIGHT) gfxAddDocumentLine(document, "\ttmpvec = -elf_LightSpotDirection;");
...
{
	if(config->vertexColor) gfxAddDocumentLine(document, "\telf_VertexColor = elf_ColorAttr;");
...
{
	if(config->textures & GFX_COLOR_MAP) gfxAddDocumentLine(document, "uniform sampler2D elf_ColorMap;");
	if(config->textures & GFX_NORMAL_MAP) gfxAddDocumentLine(document, "uniform sampler2D elf_NormalMap;");
	if(config->textures & GFX_SPECULAR_MAP) gfxAddDocumentLine(document, "uniform sampler2D elf_SpecularMap;");
	if(config->textures & GFX_HEIGHT_MAP) gfxAddDocumentLine(document, "uniform sampler2D elf_HeightMap;");
	if(config->textures & GFX_COLOR_RAMP_MAP) gfxAddDocumentLine(document, "uniform sampler2D elf_ColorRampMap;");
	if(config->textures & GFX_LIGHT_MAP) gfxAddDocumentLine(document, "uniform sampler2D elf_LightMap;");
	if(config->light && config->textures & GFX_SHADOW_MAP) gfxAddDocumentLine(document, "uniform sampler2DShadow elf_ShadowMap;");
	if(config->textures & GFX_CUBE_MAP) gfxAddDocumentLine(document, "uniform samplerCube elf_CubeMap;");
	if(config->textures & GFX_HEIGHT_MAP) gfxAddDocumentLine(document, "uniform float elf_ParallaxScale;");
}
...
void gfxAddFragmentLightingUniforms(gfxDocument* document, gfxShaderConfig* config)
{
	...
	if(config->light && config->light != GFX_SUN_LIGHT) gfxAddDocumentLine(document, "uniform float elf_LightFadeRange;");
	...
	if(config->light == GFX_SPOT_LIGHT) gfxAddDocumentLine(document, "uniform float elf_LightOuterConeCos;");
}

void gfxAddFragmentVaryings(gfxDocument* document, gfxShaderConfig* config)
{
	if(config->textures) gfxAddDocumentLine(document, "varying vec2 elf_TexCoord;");
	if(config->light && config->textures & GFX_SHADOW_MAP) gfxAddDocumentLine(document, "varying vec4 elf_ShadowCoord;");
	if(config->textures & GFX_CUBE_MAP) gfxAddDocumentLine(document, "varying vec3 elf_CubeMapCoord;");
	if(config->light || config->textures & GFX_HEIGHT_MAP) gfxAddDocumentLine(document, "varying vec3 elf_EyeVector;");
	if(config->light && config->textures & GFX_NORMAL_MAP) gfxAddDocumentLine(document, "varying vec3 elf_LightTSDirection;");
...
{
	if(config->light || config->textures & GFX_HEIGHT_MAP) gfxAddDocumentLine(document, "\tvec3 E = normalize(elf_EyeVector);");
	if(config->textures & GFX_HEIGHT_MAP)
	{
		...
		gfxAddDocumentLine(document, "\tdepth = (depth+texture2D(elf_HeightMap, elf_TexCoord+elf_HeightTexCoord).r)*0.5;");
...
{
	if(config->light && config->textures & GFX_NORMAL_MAP && config->textures & GFX_HEIGHT_MAP) gfxAddDocumentLine(document, "\tvec3 N = normalize(texture2D(elf_NormalMap, elf_HeightTexCoord).rgb*2.0-1.0);");
	if(config->light && config->textures & GFX_NORMAL_MAP && !(config->textures & GFX_HEIGHT_MAP)) gfxAddDocumentLine(document, "\tvec3 N = normalize(texture2D(elf_NormalMap, elf_TexCoord).rgb*2.0-1.0);");
	...
	if(config->light && config->light != GFX_SUN_LIGHT) gfxAddDocumentLine(document, "\tfloat att = clamp(1.0-max(elf_Distance-elf_LightRange, 0.0)/elf_LightFadeRange, 0.0, 1.0);");
	if(config->light == GFX_SPOT_LIGHT)
	{
		gfxAddDocumentLine(document, "\tvec3 D = normalize(elf_LightSpotDirection);");
		if(config->textures & GFX_NORMAL_MAP) gfxAddDocumentLine(document, "\tfloat cosCurAngle = dot(-normalize(elf_LightDirection), D);");
		else gfxAddDocumentLine(document, "\tfloat cosCurAngle = dot(-L, D);");
...
void gfxAddFragmentLightingCalcs(gfxDocument* document, gfxShaderConfig* config)
{
	if(config->light)
	{
		...
		if(config->specular)
		{
			...
			gfxAddDocumentLine(document, "\t\tfloat specStrength = clamp(pow(max(dot(R, E), 0.0), elf_Shininess), 0.0, 1.0);");
			gfxAddDocumentLine(document, "\t\tspecular = elf_SpecularColor*elf_LightColor*specStrength;");
		}
		gfxAddDocumentLine(document, "\t}");
	}
	if(!config->light) gfxAddDocumentLine(document, "\tfinalColor *= elf_DiffuseColor;");
}
...
void gfxAddFragmentTextureCalcs(gfxDocument* document, gfxShaderConfig* config)
{
	if(config->textures & GFX_HEIGHT_MAP)
	{
		if(config->textures & GFX_COLOR_MAP) gfxAddDocumentLine(document, "\tfinalColor *= texture2D(elf_ColorMap, elf_HeightTexCoord);");
		if(config->light && config->textures & GFX_COLOR_RAMP_MAP) gfxAddDocumentLine(document, "\tfinalColor.rgb *= texture2D(elf_ColorRampMap, vec2(clamp(lambertTerm, 0.0, 1.0))).rgb*elf_DiffuseColor*elf_LightColor;");
		if(config->textures & GFX_LIGHT_MAP) gfxAddDocumentLine(document, "\tfinalColor.rgb *= texture2D(elf_LightMap, elf_HeightTexCoord).rgb;");
	}
	else
	{
		if(config->textures & GFX_COLOR_MAP) gfxAddDocumentLine(document, "\tfinalColor *= texture2D(elf_ColorMap, elf_TexCoord);");
		if(config->light && config->textures & GFX_COLOR_RAMP_MAP) gfxAddDocumentLine(document, "\tfinalColor.rgb *= texture2D(elf_ColorRampMap, vec2(clamp(lambertTerm, 0.0, 1.0))).rgb*elf_DiffuseColor*elf_LightColor;");
		if(config->textures & GFX_LIGHT_MAP) gfxAddDocumentLine(document, "\tfinalColor.rgb *= texture2D(elf_LightMap, elf_TexCoord).rgb;");
	}
	if(config->textures & GFX_CUBE_MAP) gfxAddDocumentLine(document, "\tfinalColor *= textureCube(elf_CubeMap, elf_CubeMapCoord);");
}

void gfxAddFragmentPostLightingCalcs(gfxDocument* document, gfxShaderConfig* config)
{
	if(config->light && !(config->textures & GFX_COLOR_RAMP_MAP)) gfxAddDocumentLine(document, "\tfinalColor.rgb *= diffuse.rgb;");
	if(config->light && config->specular && config->textures & GFX_SPECULAR_MAP && !(config->textures & GFX_HEIGHT_MAP)) gfxAddDocumentLine(document, "\tfinalColor.rgb += specular*texture2D(elf_SpecularMap, elf_TexCoord).rgb;");
	if(config->light && config->specular && config->textures & GFX_SPECULAR_MAP && config->textures & GFX_HEIGHT_MAP) gfxAddDocumentLine(document, "\tfinalColor.rgb += specular*texture2D(elf_SpecularMap, elf_HeightTexCoord).rgb;");
	if(config->light && config->specular && !(config->textures & GFX_SPECULAR_MAP)) gfxAddDocumentLine(document, "\tfinalColor.rgb += specular;"); 
	if(config->light && config->light != GFX_SUN_LIGHT) gfxAddDocumentLine(document, "\tfinalColor.rgb *= att;");
	if(config->light && config->textures & GFX_SHADOW_MAP) gfxAddDocumentLine(document, "\tfinalColor.rgb *= shadow;");
	if(config->light == GFX_SPOT_LIGHT) gfxAddDocumentLine(document, "\tfinalColor.rgb *= spot;");
}
...
void gfxAddFragmentEnd(gfxDocument* document, gfxShaderConfig* config)
{
	if(config->fog)
	{
		if(config->blend == GFX_ADD)
			gfxAddDocumentLine(document, "\tfinalColor.rgb = mix(elf_FogColor*(finalColor.r*0.3+finalColor.g*0.59+finalColor.b*0.11), finalColor.rgb, clamp((elf_FogEnd-gl_FragCoord.z/gl_FragCoord.w)/(elf_FogEnd-elf_FogStart), 0.0, 1.0));");
		else
			gfxAddDocumentLine(document, "\tfinalColor.rgb = mix(elf_FogColor, finalColor.rgb, clamp((elf_FogEnd-gl_FragCoord.z/gl_FragCoord.w)/(elf_FogEnd-elf_FogStart), 0.0, 1.0));");
	}
	if(config->vertexColor) gfxAddDocumentLine(document, "\tfinalColor *= elf_VertexColor;");
...
void gfxAddGbufVertexAttributes(gfxDocument* document, gfxShaderConfig* config)
{
	...
	if(config->textures) gfxAddDocumentLine(document, "attribute vec2 elf_TexCoordAttr;");
	if(config->textures & GFX_NORMAL_MAP) gfxAddDocumentLine(document, "attribute vec3 elf_TangentAttr;");
	if(config->vertexColor) gfxAddDocumentLine(document, "attribute vec4 elf_ColorAttr;");
}
...
void gfxAddGbufVertexVaryings(gfxDocument* document, gfxShaderConfig* config)
{
	...
	if(config->textures) gfxAddDocumentLine(document, "varying vec2 elf_TexCoord;");
	if(config->textures & GFX_HEIGHT_MAP)gfxAddDocumentLine(document, "varying vec3 elf_EyeVector;");
...
void gfxAddGbufFragmentEnd(gfxDocument* document, gfxShaderConfig* config)
{
	if(config->textures & GFX_NORMAL_MAP)
	{
		...
		gfxAddDocumentLine(document, "\tnormal = normalize(normal.x*elf_Tangent+normal.y*elf_BiNormal+normal.z*elf_Normal);");
...
gfxShaderProgram* gfxGetGbufShaderProgram(gfxShaderConfig* config)
{
	...
	{
		gfxAddDocumentLine(document, "attribute vec3 elf_VertexAttr;");
		if(config->textures & GFX_COLOR_MAP) gfxAddDocumentLine(document, "attribute vec2 elf_TexCoordAttr;");
		gfxAddDocumentLine(document, "uniform mat4 elf_ProjectionMatrix;");
		gfxAddDocumentLine(document, "uniform mat4 elf_ModelviewMatrix;");
		if(config->textures & GFX_COLOR_MAP) gfxAddDocumentLine(document, "varying vec2 elf_TexCoord;");
		gfxAddDocumentLine(document, "void main()");
		gfxAddDocumentLine(document, "{");
		if(config->textures & GFX_COLOR_MAP) gfxAddDocumentLine(document, "\telf_TexCoord = elf_TexCoordAttr;");
		gfxAddDocumentLine(document, "\tgl_Position = elf_ProjectionMatrix*(elf_ModelviewMatrix*vec4(elf_VertexAttr, 1.0));");
		gfxAddDocumentLine(document, "}");
	}
	...
	if(config->gbuffer == GFX_GBUFFER_DEPTH)
	{
		...
		gfxAddDocumentLine(document, "void main()");
		gfxAddDocumentLine(document, "{");
		if(config->textures & GFX_COLOR_MAP) gfxAddDocumentLine(document, "\tgl_FragData[0] = texture2D(elf_ColorMap, elf_TexCoord);");
		gfxAddDocumentLine(document, "}");
	}
	else if(config->gbuffer == GFX_GBUFFER_FILL)
	{
	...
	else if(config->gbuffer == GFX_GBUFFER_LIGHTING)
	{
		...
		gfxAddDocumentLine(document, "\tvec4 vertex = elf_InvProjectionMatrix*vec4(elf_TexCoord.x*2.0-1.0, elf_TexCoord.y*2.0-1.0, depth, 1.0);");
		...
		gfxAddDocumentLine(document, "\tfloat att = clamp(1.0-max(elf_Distance-elf_LightRange, 0.0)/elf_LightFadeRange, 0.0, 1.0);");
		gfxAddDocumentLine(document, "\tfloat lambertTerm = max(dot(N, L), 0.0);");
		gfxAddDocumentLine(document, "\tif(lambertTerm > 0.0)");
		gfxAddDocumentLine(document, "\t{");
		...
		{
			if(config->light == GFX_POINT_LIGHT) gfxAddDocumentLine(document, "\tfloat str = att;");
			if(config->light == GFX_SPOT_LIGHT) gfxAddDocumentLine(document, "\tfloat str = att*spot;");
			if(config->textures & GFX_SHADOW_MAP) gfxAddDocumentLine(document, "\tstr *= shadow;");
		}
		if(config->light != GFX_SUN_LIGHT)
		{
