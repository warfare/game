...
ELF_API elfScene* ELF_APIENTRY elfCreateScene(const char* name)
{
	...
	if(name) scene->name = elfCreateString(name);
...
void elfRecursivelyImportAssets(elfScene* scene, const struct aiScene* aiscn, struct aiNode* aind)
{
	...
	for(i = 0; i < (int)aind->mNumMeshes; i++)
	{
		...
		if(mesh->mTextureCoords[0] != NULL) isTexCoords = ELF_TRUE;
	}
	...
	if(model->verticeCount > 2 && model->indiceCount > 2)
	{
		...
		for(i = 0; i < (int)aind->mNumMeshes; i++)
		{
			...
			model->areas[areaIndex].index = gfxCreateVertexData(model->areas[areaIndex].indiceCount, GFX_UINT, GFX_VERTEX_DATA_STATIC);
			...
			{
				if(realPath) elfDestroyString(realPath);
		...
		if(isTexCoords) gfxSetVertexArrayData(model->vertexArray, GFX_TEX_COORD, model->texCoords);
...
void elfDestroyScene(void* data)
{
	...
	if(scene->filePath) elfDestroyString(scene->filePath);

	if(scene->entityQueue) elfDecRef((elfObject*)scene->entityQueue);
	...
	if(scene->materials) elfDecRef((elfObject*)scene->materials);
	if(scene->textures) elfDecRef((elfObject*)scene->textures);
	if(scene->cameras) elfDecRef((elfObject*)scene->cameras);
	if(scene->entities) elfDecRef((elfObject*)scene->entities);
	if(scene->lights) elfDecRef((elfObject*)scene->lights);
	if(scene->armatures) elfDecRef((elfObject*)scene->armatures);
	if(scene->particles) elfDecRef((elfObject*)scene->particles);
	if(scene->sprites) elfDecRef((elfObject*)scene->sprites);

	elfDestroyPhysicsWorld(scene->world);
	elfDestroyPhysicsWorld(scene->dworld);

	if(scene->pak) elfDecRef((elfObject*)scene->pak);

	if(scene->composeFogShdr) gfxDestroyShaderProgram(scene->composeFogShdr);
...
void elfSetActorScene(elfScene* scene, elfActor* actor)
{
	...
	if(actor->scene) elfRemoveSceneActorByObject(actor->scene, actor);

	actor->scene = scene;

	if(actor->object) elfSetPhysicsObjectWorld(actor->object, scene->world);
...
elfCamera* elfGetOrLoadCameraByName(elfScene* scene, const char* name)
{
	...
	if(scene->pak)
	{
		...
		if(index)
		{
			...
			if(camera) elfAddSceneCamera(scene, camera);
			fclose(file);
			return camera;
		}
	}
...
elfEntity* elfGetOrLoadEntityByName(elfScene* scene, const char* name)
{
	...
	if(scene->pak)
	{
		...
		if(index)
		{
			...
			if(entity) elfAddSceneEntity(scene, entity);
			fclose(file);
			return entity;
		}
	}
...
elfLight* elfGetOrLoadLightByName(elfScene* scene, const char* name)
{
	...
	if(scene->pak)
	{
		...
		if(index)
		{
			...
			if(light) elfAddSceneLight(scene, light);
			fclose(file);
			return light;
		}
	}
...
elfParticles* elfGetOrLoadParticlesByName(elfScene* scene, const char* name)
{
	...
	if(scene->pak)
	{
		...
		if(index)
		{
			...
			if(particles) elfAddSceneParticles(scene, particles);
			fclose(file);
			return particles;
		}
	}
...
elfSprite* elfGetOrLoadSpriteByName(elfScene* scene, const char* name)
{
	...
	if(scene->pak)
	{
		...
		if(index)
		{
			...
			if(sprite) elfAddSceneSprite(scene, sprite);
			fclose(file);
			return sprite;
		}
	}
