...
gfxShaderProgram* gfxCreateShaderProgram(const char* vertex, const char* fragment)
{
	...
	if(vertex) glAttachShader(shaderProgram->id, myVertexShader);
	if(fragment) glAttachShader(shaderProgram->id, myFragmentShader);
...
{
	if(shaderProgram->id) glDeleteShader(shaderProgram->id);
...
{
	if(shaderProgram->next) gfxDestroyShaderPrograms(shaderProgram->next);
